		...
		}
		{
			...
			try 
					(Connection connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/sims_db",
								"sims", "admin123");
						PreparedStatement updateStatement = connection.prepareStatement("UPDATE inventory_of_supplies set product_code = ?, item_name = ?, item_type = ?, quantity = ?, item_condition = ?, price = ? where product_code = ?")) {
						...
						updateStatement.setLong(1, code);
						updateStatement.setString(2, name);
						updateStatement.setString(3, strItemTypeInput);
						updateStatement.setInt(4, quantity);
						updateStatement.setString(5, strItemConditionInput);
						updateStatement.setDouble(6, price);
...
public void initializeWithAccountId(long accountID) {
		
		try(Connection connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/sims_db",
				"sims", "admin123");
				PreparedStatement retrieveAccountByIdStatement = connection.prepareStatement("SELECT * FROM inventory_of_supplies WHERE product_code = ?")){
				
				retrieveAccountByIdStatement.setLong(1, accountID);
				retrieveAccountByIdStatement.execute();
				
				ResultSet accountResult = retrieveAccountByIdStatement.getResultSet();
				accountResult.next();
				
				accountIdCurrentlyBeingUpdated = accountResult.getInt("product_code");
				
				jtxtfldProductCode.setText(accountResult.getString("product_code"));
				jtxtfldItemName.setText(accountResult.getString("item_name"));
				String strItemTypeInput = accountResult.getString("item_type");
				jtxtfldQuantity.setText(accountResult.getString("quantity"));
				jtxtfldPrice.setText(accountResult.getString("price"));
				String strItemConditionInput = accountResult.getString("item_condition");
				


				if (strItemTypeInput.equals("Stationery"))
					jrdbtnStationery.setSelected(true);
				else if (strItemTypeInput.equals("Consumables"))
					jrdbtnConsumables.setSelected(true);
				else if (strItemTypeInput.equals("Others"))
					jrdbtnOthers.setSelected(true);
				

				if (strItemConditionInput.equals("Good Condition"))
					jrdbtnGoodCondition.setSelected(true);
				else if (strItemConditionInput.equals("Bad Condition"))
					jrdbtnBadCondition.setSelected(true);
				else if (strItemConditionInput.equals("Needs Attention"))
					jrdbtnNeedsAttention.setSelected(true);
				else if (strItemConditionInput.equals("Out Of Stock"))
					jrdbtnOutOfStock.setSelected(true);
